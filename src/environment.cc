...
uint32_t Environment::getQuadId(Point2D position)
{
	...
	if( quadId >= gridData._quadX * gridData._quadY ){
		std::cout << std::endl;
		std::cout << g_currTimeSim << "\t" << "****BUG****" << std::endl;
...
{
	agent->setDensity(
		 (double)agent->getAgentNeighbors().size() / ( 3.14*(g_attractionRadius - agent->radius())*(g_attractionRadius - agent->radius()) )
			 );
}
...
{
	/*
	if(g_elevationPatchDataValid && this->haveElevation()){
...
void Environment::updateQuads()
{
	...
	if(this->getFloodParams().enable && g_currTimeSim*g_deltaT >= this->getFloodParams().arrivalTime ){
		#pragma omp parallel for 
		for(uint32_t zId = 0; zId < this->getFloodZones().size(); zId++ ){
			// por cada zona inundable, rescata los patch agents respectivos
			Zone floodZone = this->getFloodZone(zId);
...
void Environment::updateStats()
{
	...
	statsDeceasedAgents << g_currTimeSim <<  ":" ;
	...
	g_logDeceasedAgents.push_back(statsDeceasedAgents.str());
	
	std::ostringstream statsSIR;
	statsSIR << g_currTimeSim <<  ":" ;
	...
	g_logSIRpanic.push_back(statsSIR.str());
